...
The bot can invoke the CLI from a checkout or call the two HTTP routes in [commands.md](commands.md). Load [the procurement playbook](../playbooks/README.md). Keep the checkout revision with the job's saved context and use stable command keys. Query live status through the service, not Git history.

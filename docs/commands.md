...
## Per-product recommendation coverage

`quote.add.data.recommendation` lists exact products/providers in a candidate cart, separately from the user’s request items. Entries use `key`, `itemKey`, `subject`, and `target`. Reviews link via `recommendationKey` and must match the target, subject and request item. Every target needs its own observations or `reviewGap: {reason, sources}` documenting where reviews were sought. Each entry includes `comparison: {rationale, alternatives: [{target, reason, sources}]}` or an empty alternatives array with `noAlternative: {reason, sources}` explaining the search. Optional fields allow incomplete drafts; omitted coverage cannot be reported as complete research.

Queries and quote write responses expose `research.reviewCoverage` and `research.researchPermission`. Missing coverage/comparisons route to the agent, do not require human approval and introduce no new purchasing gates. Existing authorization, safety and final-cost controls still apply. The server validates links and evidence structure; it cannot detect an omitted product in free text, judge a rationale’s quality, verify external reviews, or attest subagent execution.

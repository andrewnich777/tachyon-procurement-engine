...
export function assessResearch(s:State,quote:Quote|null,now=new Date()) {
  ...
  checklist.push({criterion:'landed-cost',label:'Final cost within request budget',required:true,
    status:amount===null?'unresolved':q?.currency!==s.data.currency || (s.data.budgetCents!==null&&amount>s.data.budgetCents)?'contradicted':'supported',
    finding:amount===null?'Resolve shipping, tax, hazmat and other charges.':`${amount} ${q!.currency} minor units; ${s.data.budgetCents===null?'no request budget provided':'request budget '+s.data.budgetCents}.`,sources:q?.sources??[]});
  ...
  checklist.push({criterion:'deadline',label:'Delivery fits the requested deadline',required:true,
    status:!s.data.neededBy||!timing.forecast?'unresolved':timing.late?'contradicted':'supported',

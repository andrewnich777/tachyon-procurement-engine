...
The local service binds to `127.0.0.1:47831`. A cloud bot needs a reachable HTTPS service through the operator's existing connection. No public app hosting, custom Slack adapter, or model API is required by this codebase.
...
Integration tests require the development database configured in `.env.local`. They create isolated fictional workspaces and retain append-only test history; they do not delete operational records. The suite exercises the actual Postgres transactions and HTTP boundary, including caps under concurrency, retries, unknown outcomes, revoked evidence, stale approvals, partial receipts, and replay.
...
| `migrations/` | Versioned schema and integrity constraints |
